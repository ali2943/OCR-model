...
from inference.predict_trocr import CustomTrOCR
...
app = FastAPI(
    title="OCR API (TrOCR)",
    description="REST API for text recognition using fine-tuned TrOCR model",
    version="2.0.0"
)
...
def get_ocr_model() -> CustomTrOCR:
    ...
    if ocr_model is None:
        model_dir = os.getenv('MODEL_DIR', './trocr_model/final')
...
async def root():
    ...
    <html>
    <head>
        <title>OCR API (TrOCR)</title>
    </head>
    <body>
        <h1>OCR API (TrOCR) is running!</h1>
...
def main():
    ...
    parser.add_argument('--model_dir', type=str, default='./trocr_model/final',
                        help='Directory containing trained TrOCR model')
